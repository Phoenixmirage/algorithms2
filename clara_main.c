...
int main(int argc, char** argv){
    ...
    if (!strcmp(metric_space, "vector")){
		...
		if(!strcmp(metric,"euclidean")){
			...
			init_euc_cl(&temp,no_cl);
			
		    for(i=0;i<s;i++){			    	
			    pam_init(no_cl,size,n_sample,&objects,&clusters);
			    printf("init done\n");
			    euc_pam_ass(clusters,no_cl,n_sample,objects,size);
			    printf("assign done\n");
			    ...
			    euc_pam_ass(clusters,no_cl,items,whole,size);			//assignment with the full table    
			...
			euc_pam_ass(temp,no_cl,items,whole,size);
