...
List_nodes* Euclidean_input(FILE *fd,int* final_size, int * item){
		...
		while(!feof(fd)){
			...
			tempnod->point.center=0;                                                    //fill list of items
			for(i=0;i<size;i++)
			{
	...
	for(j=0;j<no_cl;j++){				//for every center
		for (t=0;t<items;t++){			//for every non centroid
			...
			for(i=0;i<items;i++){
				if(i!=t){
					if(array[i]->center==j){
						distance1=euclidean_distance(array[i]->array,array[t]->array,size);   			//dist(i,t)
			...
			if(sum<0){																//if sum is negative then the total cost is lower
				...
				euc_pam_ass((*clusters),no_cl,items,array,size);					//assign 
			}
		}
	}
